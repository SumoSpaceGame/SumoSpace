...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] UserNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is UserNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (UserNetworkObject.Length > 0 && UserNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(UserNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<UserBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateUser instead, its shorter and easier to type out ;)")]
		public UserBehavior InstantiateUserNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(UserNetworkObject[index]);
			var netBehavior = go.GetComponent<UserBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<UserBehavior>().networkObject = (UserNetworkObject)obj;
		...
		/// </summary>
		/// <returns>
		/// A local instance of UserBehavior
		/// </returns>
		/// <param name="index">The index of the User prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public UserBehavior InstantiateUser(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			if (UserNetworkObject.Length <= index)
			{
				Debug.Log("Prefab(s) missing for: User. Add them at the NetworkManager prefab.");
				return null;
			}
